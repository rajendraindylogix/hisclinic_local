...
<?php else: ?>
    <h2><a class="link" href="<?php echo esc_url(admin_url('post.php?post=' . $order->get_id() . '&action=edit')); ?>"><?php printf(__('Order #%s', 'woocommerce'), $order->get_order_number()); ?></a> (<?php printf('<time datetime="%s">%s</time>', wc_format_datetime($order->get_date_created(), 'c'), wc_format_datetime($order->get_date_created())); ?>)</h2>
<?php endif; ?>
