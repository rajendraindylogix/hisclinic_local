...
class RP_WCEC_Macros
{
    ...
    public static function filter_list_by_keys($macros, $keys)
    {
        ...
        foreach ($keys as $key) {
            ...
            // Order meta
            if (RightPress_Help::string_contains_phrase($key, 'order_meta_')) {
                $custom_macros[$key] = array(
                    'order_meta' => substr($key, 11),
                );
            }
            // User meta
            else if (RightPress_Help::string_contains_phrase($key, 'user_meta_')) {
                $custom_macros[$key] = array(
    ...
    public static function replace($string = '', $macros = array(), $object = null, $args = array(), $content_type = 'text/plain', $recipient_email = null)
    {
        ...
        foreach ($macros as $macro_key => $macro) {
            ...
            // Order meta used to retrieve value
                ...
                $value = RightPress_WC::order_get_meta($object, $macro['order_meta'], true);
            }
            // WordPress user data used to retrieve value
                ...
                if (RightPress_Help::is_empty($value) && in_array($property, array('first_name', 'last_name'), true)) {
    ...
    public static function get_user_id_from_args($object, $args)
    {
        ...
        else if (is_a($object, 'WC_Order') && $object->get_customer_id()) {
            return $object->get_customer_id();
        }
        else if (isset($args['order']) && is_a($args['order'], 'WC_Order') && $args['order']->get_customer_id()) {
            return $args['order']->get_customer_id();
        }
    ...
    public static function get_order_macro_list()
    {
        return array(
            ...
            'order_notes' => array(
                'custom' => 'macro_order_notes_value',
            ),
            'order_number' => array(
    ...
    public static function macro_order_state_value($property, $order)
    {
        ...
        if (is_a($order, 'WC_Order')) {
            ...
            // Get state
            if ($state = $order->$method()) {

                $method = 'get_' . ($property === 'billing_state' ? 'billing_country' : 'shipping_country');

                // Get country
                if ($country = $order->$method()) {
    ...
    {
        if (is_a($order, 'WC_Order')) {
            if ($order_date = wc_format_datetime($order->get_date_created())) {
                return $order_date;
            }
        }
    ...
    public static function macro_order_order_customer_details_value($order, $args = array(), $content_type = 'text/plain')
    {
        ...
        if (is_a($order, 'WC_Order')) {
            ...
            if (RP_WCEC_Styling::are_styling_options_in_use() && RP_WCEC_Styling::are_wc_templates_overriden()) {
                ...
                // Include template
                RightPress_Help::include_template($template, RP_WCEC_PLUGIN_PATH, 'woocommerce-email-center', array(
                    'order'         => $order,
     ...
     * @return string
     */
    public static function macro_order_notes_value($order, $args, $content_type = 'text/plain')
    {
        ...
        if (is_a($order, 'WC_Order')) {
            ...
            if ($order_notes = $order->get_customer_order_notes()) {
                ...
                $order_notes = wp_list_pluck($order_notes, 'comment_content');

                // Join order notes to string
                $order_notes = implode(($content_type === 'text/plain' ? "\n" : '<br>'), $order_notes);

                // Return formatted order notes
                return $order_notes;
            }
        }
    ...
    public static function macro_user_auto_login_link_value($object, $args, $content_type = 'text/plain', $recipient_email = null)
    {
        ...
        if ($user = get_user_by('email', $recipient_email)) {
            ...
            if (!$key) {
                $key = RightPress_Help::get_hash(true);
    ...
    public static function macro_user_password_reset_link_value($object, $args, $content_type = 'text/plain')
    {
        ...
        // Unable to get user id and required arguments are not set
        if (!$user_id && (empty($args['password_reset_key']) || empty($args['user_login']))) {
            return '';
        }
        ...
        $password_reset_key = !empty($args['password_reset_key']) ? $args['password_reset_key'] : get_password_reset_key($user_data);

        // Get user login
        $user_login = !empty($args['user_login']) ? $args['user_login'] : $user_data->user_login;

        // Return password reset url
        $wc_endpoint_url = wc_get_endpoint_url('lost-password', '', wc_get_page_permalink('myaccount'));
        return esc_url(add_query_arg(array('key' => $password_reset_key, 'login' => rawurlencode($user_login)), $wc_endpoint_url));
    }
    ...
    {
        return strip_tags(wc_price($order->get_shipping_total(), array('currency' => $order->get_currency())));
    }
