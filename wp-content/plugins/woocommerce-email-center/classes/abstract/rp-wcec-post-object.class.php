...
class RP_WCEC_Post_Object
{
    ...
    public function render_meta_box_settings($post)
    {
        ...
        // Load view
        include RP_WCEC_PLUGIN_PATH . 'views/' . $object->get_post_type_short() . '/settings.php';
    }
    ...
    public function render_meta_box_actions($post)
    {
        ...
        // Load view
        include RP_WCEC_PLUGIN_PATH . 'views/' . $object->get_post_type_short() . '/actions.php';
    ...
    {
        include RP_WCEC_PLUGIN_PATH . 'views/shared/preloader.php';
    }
    ...
    public function get_valid_list_action_object($action)
    {
        ...
        if (!isset($_REQUEST['wcec_object_id']) || !RightPress_Help::post_type_is($_REQUEST['wcec_object_id'], $this->get_post_type())) {
            return false;
        }
    ...
    public function populate()
    {
        ...
        if ($this->is_configurable() && method_exists($this, 'get_post_type')) {
            ...
            $this->status = (isset($post_terms[0]) && is_object($post_terms[0])) ? RightPress_Help::clean_term_slug($post_terms[0]->slug) : null;
