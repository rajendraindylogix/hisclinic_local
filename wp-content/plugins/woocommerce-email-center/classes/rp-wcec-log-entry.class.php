...
class RP_WCEC_Log_Entry extends RP_WCEC_Post_Object
{
    ...
    public function __construct($id = null, $params = array())
    {
        ...
        if ($id === null && !empty($params)) {
            ...
            $this->iso_date = RightPress_Help::get_iso_datetime();
    ...
    public function print_list_column_value($column_key)
    {
        ...
        switch ($column_key) {
            ...
            case 'log_time':
                ...
                $date_time = RightPress_Help::get_adjusted_datetime($this->time, RP_WCEC::get_date_time_format());
            ...
            case 'log_trigger':
                if ($this->trigger_id) {
                    if (!RightPress_Help::post_is_trashed($this->trigger_id)) {
                        RightPress_Help::print_link_to_post($this->trigger_id, RP_WCEC_Trigger::get_trigger_title_statically($this->trigger_id), '', '', 10);
                    }
                    else {
            ...
            case 'log_email':
                if ($this->email_id) {
                    if (!RightPress_Help::post_is_trashed($this->email_id)) {
                        RightPress_Help::print_link_to_post($this->email_id, $this->email_subject, '', '', 10);
                    }
                    else {
            ...
            case 'log_order':
                if ($this->order_id) {
                    ...
                    if (!RightPress_Help::post_is_trashed($this->order_id) && RightPress_Help::post_type_is($this->order_id, 'shop_order')) {
                        $order = wc_get_order($this->order_id);
                        // WC31: Orders will no longer be posts
                        RightPress_Help::print_link_to_post($this->order_id, ('#' . $order->get_order_number()));
                    }
                    else {
    ...
    public function populate_own_properties()
    {
        ...
        foreach (array('status', 'log_action') as $taxonomy) {
            ...
            // Set property
            $this->$taxonomy = (isset($post_terms[0]) && is_object($post_terms[0])) ? RightPress_Help::clean_term_slug($post_terms[0]->slug) : null;

            // Set title property
            $title_key = $taxonomy . '_title';
            $this->$title_key = $this->$taxonomy ? $this->get_term_title_from_slug($taxonomy, $this->$taxonomy) : null;
        }
