...
class RP_WCEC_Trigger extends RP_WCEC_Post_Object
{
    ...
    public function print_list_column_value($column_key)
    {
        switch ($column_key) {
            ...
            case 'trigger_title':
                RightPress_Help::print_link_to_post($this->get_id(), $this->get_title(), '<span class="rp_wcec_row_title_cell">', '</span>');
            ...
            case 'trigger_email':
                ...
                if (!empty($this->email_id) && ($email = RP_WCEC_Post_Object::cache($this->email_id))) {
                    RightPress_Help::print_link_to_post($email->get_id(), $email->get_subject());
                }
                else {
    ...
    public function output_hidden_templates()
    {
        ...
        // Include templates
        include RP_WCEC_PLUGIN_PATH . 'views/shared/conditions_templates.php';
    }
    ...
    public static function get_object_properties_js($post)
    {
        ...
        if (is_object($post) && !empty($post->ID) && RightPress_Help::post_type_is($post->ID, self::$post_type)) {
    ...
    public static function get_selected_option_labels($post)
    {
        ...
        // Get trigger
        if (is_object($post) && isset($post->ID) && RightPress_Help::post_type_is($post, self::$post_type)) {
    ...
    public function post_deleted($post_id)
    {
        ...
        if (RightPress_Help::post_type_is($post_id, self::$post_type)) {
